import { RecordDispatch, RecordForm, Records } from "../../types/records";
import api from "../../utils/api";

export const getRecords = () => async (dispatch: RecordDispatch) => {
    dispatch({ type: 'GET_RECORDS_START' });
    try {
        const response = await api().get<Records[]>("/records");
        response.data.sort((a, b) => b.id - a.id);
        dispatch({ type: "GET_RECORDS_SUCCESS", payload: response.data });
    } catch {
        dispatch({ type: "GET_RECORDS_ERROR" });
    }
}

export const addRecord =
    (form: RecordForm) => async (dispatch: RecordDispatch) => {
        dispatch({ type: "ADD_RECORDS_START" });
        try {
            const response = await api().post<Records>("/records", form);
            dispatch({ type: "ADD_RECORDS_SUCCESS", payload: response.data });
        } catch {
            dispatch({ type: "ADD_RECORDS_ERROR" });
        }
    };

export const updateRecord = (form: RecordForm, updatedId: Records['id']) => async (dispatch: RecordDispatch) => {
    dispatch({ type: 'UPDATE_RECORDS_START' });
    try {
        const response = await api().put<Records>(`/records/${updatedId}`, form);
        dispatch({ type: "UPDATE_RECORDS_SUCCESS", payload: response.data });
    } catch {
        dispatch({ type: "UPDATE_RECORDS_ERROR" });
    }
}

export const deleteRecord = (deletedId: Records['id']) => async (dispatch: RecordDispatch) => {
    dispatch({ type: 'DELETE_RECORDS_START' });
    try {
        await api().delete(`/records/${deletedId}`);
        dispatch({ type: "DELETE_RECORDS_SUCCESS", payload: deletedId });
    } catch {
        dispatch({ type: "DELETE_RECORDS_ERROR" });
    }
}